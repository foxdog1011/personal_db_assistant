...
export function registerRunBriefTool(server: any, ctx: ToolContext) {
  const inputSchema = {
    type: "object",
    properties: {
      task: { type: "string" },
  ...
  server.tool?.("run_brief", { description: "Run Knowledge Task (brief mode)", inputSchema }, async (req: any) => {
